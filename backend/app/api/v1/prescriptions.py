...
async def create_prescription(
    payload: PrescriptionCreate, current_user: CurrentUser, session: DbSession, settings: AppSettings
) -> PrescriptionRead:
    prescription = await PrescriptionService(session, settings).create_prescription(
...
async def list_prescriptions(
    current_user: CurrentUser, session: DbSession, settings: AppSettings, patient_id: uuid.UUID | None = None
) -> list[PrescriptionRead]:
    prescriptions = await PrescriptionService(session, settings).list_my_prescriptions(
...
async def get_prescription(
    prescription_id: uuid.UUID, current_user: CurrentUser, session: DbSession, settings: AppSettings
) -> PrescriptionRead:
    prescription = await PrescriptionService(session, settings).get_my_prescription(
